...
<div class="container">
				...
				<tbody>
					...
					foreach ($data as $row) {
						...
						<tr>
							...
							<td><img style="width: 100px;height: 130px; border: 1px grey solid;" src="public/images/product/<?php echo $row['product_image'] ?>"></td>

							<td>
								<a href="?mod=sale&act=add2cart&product_code=<?php echo $row['product_code']; ?>" class="btn btn-info">
	...
	<div style="width: 50%; float: right;border: 1px #EEEEEE solid;">
		...
		<div style="width: 90%; margin: 5% auto 5%;">
			...
			<p>Địa chỉ : <?php  echo $customer['customer_address']; ?></p>
			<p>Số điện thoại liên hệ : <?php  echo $customer['customer_mobile']; ?></p>
				...
				<tbody>
					...
					if(isset($_SESSION['cart'])){
						...
						foreach ($product_buy as  $product) { 
							$tong_tien+=$product['product_price']*$product['product_quantity'];
							?>
							...
							<tr>
								...
								<td align="right">
									<?php echo number_format($product['product_price']*$product['product_quantity']) ?>
								</td>
								<td>
									<a href="?mod=sale&act=add2cart&product_code=<?php echo $product['product_code']; ?>"  class="btn btn-success">+</a>
									<a href="?mod=sale&act=remove_product&product_code=<?php echo $product['product_code']; ?>"  class="btn btn-danger">-</a> 
									<a href="?mod=sale&act=unset_product&product_code=<?php echo $product['product_code']; ?>"  class="btn btn-warning">Hủy</a> 
								</td>
