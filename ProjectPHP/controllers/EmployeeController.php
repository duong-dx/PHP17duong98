...
class EmployeeController{
	...
	function store(){
        ...
        if (move_uploaded_file($_FILES["employee_image"]["tmp_name"], $target_file)) { // nếu upload file không có lỗi 
            ...
            $data['employee_image']=basename($_FILES["employee_image"]["name"]);
        } else { // Upload file có lỗi 
        	// echo "Sorry, there was an error uploading your file.";
            $data['employee_image']="imgdefault.jpg";
        }
    ...
    unset($data['submit']);
    $data['employee_password']= md5($_POST['employee_password']);
	...
	function statistical(){
		...
		foreach ($dem_product as  $value) {
			if ($value['product_quantity']==0) {
			 ...
			 else if($value['product_quantity']<10){
			 	$saphet++;

			 }
			 else if($value['product_quantity']>20){
			 	$tonkho++;
		...
		$employee_birthday_today=$this->model->birthday($today_month,$today_day);
			// echo "<pre>";
		 // 		print_r($employee_birthday_today);
		 // 	echo "</pre>";
		 // 	// die;
		$customer_birthday_today=$this->customer->birthday($today_month,$today_day);
		// echo $tonkho;
		// echo $hethang;
		// echo $saphet;
